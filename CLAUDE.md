...
`<home>/sdk/` is the runtimes' home, in claude's formats: `sdk/settings.json` (`permissions.allow`/`deny`/`additionalDirectories`, `outputStyle`, `autoMemoryEnabled`/`autoMemoryDirectory`, `enabledPlugins`) and `sdk/.winter.json` (MCP servers: `mcpServers` is the user scope, `projects[<root>].mcpServers` the local scope — the default of `winter mcp add`/`mcp.add`; a trusted project's `.winter/mcp.json` is the project scope, precedence local > project > user, and the repo-root `.mcp.json` is no longer read). Those keys moved out of `settings.json`: the daemon reads them only through the `sdk…` readers in `settings.ts` (`liveSettingsView` strips them from every live holder), writes go through `sdk-files.ts` (atomic, 0600, never over a file that does not parse), and the **settings split** (`migration/settings-split.ts`) copies each key once — per-key marker AND sdk-key-absent — at every boot on every home, never writing `settings.json` (its copies stay for a downgrade). The runtimes' STORE (`projects`, `skills`, `agents`, `file-history`, …) is `storeHomeFor(home)`: `<home>/sdk` when the linked router applies run homes, `<home>` on router 0.0.11.

**Run homes.** When the linked router exports `buildRunHome` (feature detection, `runtime-sdk/run-home-support.ts`), every incarnation on either leg awaits a per-run folder `<home>/cache/runs/<id>` (`runHomeInputFor`), built last so a refusal never leaves one behind, and the daemon stops stating what the folder carries: no `agents`/`plugins`/`skills`/`outputStyle`/`brand`, no saved allow rules, no project instructions/rules/output style, none of the five router-owned `WINTER_*` variables, and `settingSources: ["user"]` (the router refuses each typed; `isRunHomeError` forwards its code as `data.code`). A folder is disposed only when the router says `safe`; a quarantined one is kept and recorded (`run_root_quarantine`, schema v7); a failed open disposes at once. Boot recovery reconciles recorded roots, leftover run folders and stale `claude-resume-*` roots through the router's `reconcileRootForRecovery` BEFORE deleting anything (`runtime-state/root-recovery.ts`, run late at the runtime-sdk site like §13 step 10). On router 0.0.11 none of this runs and every child launches as before.

**Migration C** (`migration/migrate-c.ts`) moves an old-layout home (real `projects/` or `skills/` content) into `sdk/` in two phases: phase 1 at the boot hook, after the lock and the settings load (preflight — a live lease, an unexpected `sdk/`, an unreconcilable official working copy or unconvertible plugins refuse BOOT — then move-dirs with relative compatibility links, copy-files, the split, convert-plugins, runtime-state), and phase 2 at the late site through the router's door (the official working copies, the archive, the done marker). Only the profile's default home migrates by itself; any other old-layout home refuses `sdk_home_migration_required` until `winter migrate --sdk-home --home <dir>`, and a half-migrated one refuses `sdk_home_half_migrated` on every build until `--resume`/`--rollback`. Rollback never reverts what the user did after the upgrade. Stored model tags are never rewritten: `canonicalModelTag` (`runtime-sdk/model-tag.ts`) resolves a catalog rename ON READ — the live settings view, the runtime-state row decoder and `ModelTagSchema`'s transform — and only when the old tag no longer resolves and the new one does.
