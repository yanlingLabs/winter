...
A role with no explicit pin follows `settings.provider.model`'s provider when it is eligible AND credentialed (so a DeepSeek user with a DeepSeek key gets titles on DeepSeek with zero setup), else the first credentialed member of `internalProviderPreferenceOrder()` (`INTERNAL_PROVIDER_IDS` first, then `credentialInventory()` order) — never Claude. Its default MODEL is the provider's own `terra`/`luna` family slot when it has one, else `settings.provider.model` when this IS its provider, else the provider's first non-blocked llm row.
