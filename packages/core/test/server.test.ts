...
describe("daemon IPC", () => {
  ...
  // WS-21 (L4 request 2): the legacy `<home>/plugins` consent-seam tests (skills scanned from the
  // plugin dir, MCP servers started by the daemon) are retired with that wiring — a plugin's skills,
  // MCP servers and hooks are claude-native content the runtimes load (lane L4 covers the plugin RPCs).
