...
export function createInternalProviderView(deps: {
  ...
  const view: InternalProviderView = {
    ...
    async refresh() {
      ...
      log(
        next.size === 0
          ? `internal-provider: no provider Winter's own jobs can run on holds a credential (was ${describe(before)}) — titles, the bash safety reviewer, the dreamer and the session cleaner are inert until one is stored (\`winter login\` for ChatGPT/Codex, \`winter credentials set <provider>\` for an API key)`
          : `internal-provider: Winter's own jobs can run on ${describe(next)} (was ${describe(before)})`,
      );
    },
  };
  return view;
