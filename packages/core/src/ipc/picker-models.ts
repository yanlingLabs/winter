...
// back would be a cycle). Both read the SAME pinned-catalog row, so they can never disagree.
 ...
 * them. `console` is its own provider id in the catalog and is checked on its own terms.
 */
export function pickerModels(deps: { credentials: CredentialPresence; home: string }): SyncConfigModel[] {
  ...
  for (const provider of catalog.providers) {
    const present = provider.id === "console" ? consolePresent : deps.credentials.byProvider[provider.id] !== undefined;
    if (!present) continue;
